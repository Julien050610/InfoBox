...
工作台支持标题、摘要、标签和正文全文检索；“筛选”面板可先选择 Library 或任意文件夹，再组合文件类型、多个标签（全部/任一）、入库状态、阅读状态、收藏、最低评分、发表日期、收件日期和排序方式。应用条件后主标题显示“筛选结果”，副标题显示筛选文件夹。当前搜索范围与筛选条件可保存为左栏的“筛选视图”；筛选视图只是一个可重复使用的查询入口，不会复制或移动资料。右栏可切换收藏和未读/阅读中/已读状态，也能建立“相关、引用、支持、反驳、后续研究”关系。
